...
def display_plan(
    ...
    print_header(f"PIXERA INTAKE — PLAN PHASE")
...
def execute_plan(plans: list[FilePlan], show_root: Path) -> ExecutionResult:
    ...
    for i, plan in enumerate(plans, start=1):
        ...
        n_of_m = f"{i}/{total}" if total else "?"
...
def run_intake(show_root: Path, config: ShowConfig) -> None:
    ...
    append_to_delivery_log(show_root, source, result, conflicts_count)
...
def _validate_specs(
    ...
    if specs.framerate is not None:
        if abs(specs.framerate - float(expected.framerate)) > 0.01:
            _apply(
    ...
    # Color space
    if specs.color_space and specs.color_space != expected.color_space:
        _apply(
    ...
    # Color range
    if specs.color_range and specs.color_range != expected.color_range:
        _apply(
    ...
    # Audio sample rate
    if specs.audio_sample_rate is not None and specs.audio_sample_rate != expected.audio_sample_rate:
        _apply(
    ...
    # Audio channels
    if specs.audio_channels is not None and specs.audio_channels != expected.audio_channels:
        _apply(
