...
class RecentShow:
    ...
    show_name:      str
    last_used:      datetime
    dashboard_view: str | None = None  # "cards", "compact", or None (default behavior)
...
def load_recent_shows() -> list[RecentShow]:
    ...
    for entry in raw_entries:
        ...
        dashboard_view = entry.get("dashboard_view") or None
        shows.append(RecentShow(
            path=path,
            show_name=show_name,
            last_used=last_used,
...
def save_recent_shows(shows: list[RecentShow]) -> None:
    ...
    for s in shows[:_MAX_ENTRIES]:
        ...
        if s.dashboard_view is not None:
            entry["dashboard_view"] = s.dashboard_view
...
def add_or_update(show_path: Path, show_name: str) -> None:
    """Add a show to the recent list (or update its last_used if already present)."""
    shows = load_recent_shows()

    # Preserve dashboard_view from existing entry if present
    existing = next((s for s in shows if s.path == show_path), None)
    dashboard_view = existing.dashboard_view if existing else None

    shows = [s for s in shows if s.path != show_path]
    shows.insert(0, RecentShow(
        path=show_path,
        show_name=show_name,
    ...
    save_recent_shows(shows)


def set_dashboard_view(show_path: Path, view: str) -> None:
    """Persist the dashboard view preference for a specific show."""
    shows = load_recent_shows()
    for s in shows:
        if s.path == show_path:
